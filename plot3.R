...
windows()
with(hpcDat, plot(hpcDat$Time,hpcDat$Global_active_power,type="n",ylab="Energy 
                  Sub Metering", xlab=""))

with(hpcDat, plot(hpcDat$Time,hpcDat$Sub_metering_1,col="black", type="l"))
with(hpcDat, plot(hpcDat$Time,hpcDat$Sub_metering_2,col="blue", type="l"))
with(hpcDat, plot(hpcDat$Time,hpcDat$Sub_metering_3,col="red", type="l"))
with(hpcDat, legend("topright", ,col=c("black","blue","red"),pch = "-",
